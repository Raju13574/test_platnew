...
const UserTestResult = () => {
        ...
        if (response?.data) {
          setTestResult({
            ...
            summary: {
              ...
              maxScore: response.data.maxTotalScore,
              passingScore: response.data.passingScore,
              timeTaken: Math.floor((new Date(response.data.endTime) - new Date(response.data.startTime)) / 1000)
            },
            mcq: {
              score: response.data.mcqScore,
              maxScore: response.data.maxMcqScore,
              total: response.data.mcqResults?.length || 0,
              correct: response.data.mcqResults?.filter(q => q.isCorrect).length || 0,
              questions: response.data.mcqResults
            },
            coding: {
              score: response.data.codingScore,
              maxScore: response.data.maxCodingScore,
              total: response.data.codingResults?.length || 0,
              completed: response.data.codingResults?.filter(c => c.status === 'completed').length || 0,
              challenges: response.data.codingResults?.map(challenge => ({
                ...challenge,
                testCases: {
                  results: challenge.testCaseResults
                }
              }))
            }
          });
    ...
    <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-xl">
          <div className="text-sm text-purple-600 font-medium">MCQ Score</div>
          <div className="mt-2">
            <div className="text-3xl font-bold text-purple-900">
              {testResult.mcq?.score || 0}/{testResult.mcq?.maxScore || 0}
            </div>
            <div className="text-sm text-purple-500 mt-1">
              {testResult.mcq?.correct || 0}/{testResult.mcq?.total || 0} Questions Correct
            </div>
          </div>
        </div>

        <div className="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl">
          <div className="text-sm text-green-600 font-medium">Coding Score</div>
          <div className="mt-2">
            <div className="text-3xl font-bold text-green-900">
              {testResult.coding?.score || 0}/{testResult.coding?.maxScore || 0}
            </div>
            <div className="text-sm text-green-500 mt-1">
              {testResult.coding?.completed || 0}/{testResult.coding?.total || 0} Challenges Complete
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Score: {testResult.mcq?.score || 0}/{testResult.mcq?.maxScore || 0} | 
        ...
        <div className={`px-4 py-2 rounded-full ${
          ...
          {testResult.mcq?.score || 0}/{testResult.mcq?.maxScore || 0} Points
        </div>
      </div>
  ...
  const getChallengeStatus = (challenge) => {
    // If there's no code or test cases, it's not attempted
    if (!challenge.code || !challenge.testCaseResults?.length) {
      return 'not attempted';
    }

    // If there are syntax errors, mark as failed
    const hasSyntaxError = challenge.testCaseResults?.some(
      test => test.error?.includes('SyntaxError')
    );
    if (hasSyntaxError) {
      return 'failed';
    }

    // Calculate based on test case results
    const passedTests = challenge.testCaseResults.filter(tc => tc.passed).length;
    const totalTests = challenge.testCaseResults.length;

    if (totalTests === 0) return 'not attempted';
    if (passedTests === 0) return 'failed';
    if (passedTests === totalTests) return 'passed';
    return 'partial';
  };

  const getChallengeStatusDisplay = (status) => {
    ...
    <div className="bg-white rounded-xl shadow-sm p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Score: {testResult.coding?.score || 0}/{testResult.coding?.maxScore || 0} | 
            Completed: {testResult.coding?.completed || 0}/{testResult.coding?.total || 0}
          </p>
        </div>
        <div className={`px-4 py-2 rounded-full ${
          testResult.coding?.challenges?.every(c => c.status === 'passed') ? 'bg-green-100 text-green-800' :
          testResult.coding?.challenges?.some(c => c.status === 'failed') ? 'bg-red-100 text-red-800' :
          testResult.coding?.challenges?.some(c => c.status === 'partial') ? 'bg-yellow-100 text-yellow-800' :
          'bg-red-100 text-red-800'
        }`}>
          {testResult.coding?.challenges?.every(c => c.status === 'passed') ? 'Completed' :
           testResult.coding?.challenges?.some(c => c.status === 'failed') ? 'Failed' :
           testResult.coding?.challenges?.some(c => c.status === 'partial') ? 'Partial' :
           'Not Attempted'}
        </div>
      </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-xl font-semibold flex items-center gap-2">
                    Challenge {index + 1}
                    <span className={`px-3 py-1 rounded-full ${getChallengeStatusDisplay(challenge.status).className}`}>
                      {getChallengeStatusDisplay(challenge.status).text}
                    </span>
                  </h3>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="font-semibold mt-1">{getChallengeStatus(challenge)}</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
