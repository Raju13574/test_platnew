          ...
          <div className="mt-1 space-y-1">
            {item.children.map((child, index) => (
              <Link
                key={index}
                to={child.path}
                className={`flex items-center pl-8 pr-3 py-2 text-sm rounded-md relative
                  ${child.isComingSoon ? 'pointer-events-none' : ''}
              ...
              >
                ...
                <span className={child.isComingSoon ? 'blur-[1px]' : ''}>
                  {child.label}
                </span>
                {child.isComingSoon && (
                  <span className="absolute right-2 text-xs font-medium text-gray-400 no-blur">
                    Coming Soon
                  </span>
                )}
                {child.badge && (
