...
const CodingSection = ({ testData, setTestData }) => {
  ...
  const handleSelectFromQuestionBank = (selectedQuestions) => {
    setTestData(prev => ({
      ...prev,
      codingChallenges: [...prev.codingChallenges, ...selectedQuestions]
    }));
    toast.success(`Added ${selectedQuestions.length} coding challenges from Question Bank`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-medium">Coding Challenges</h2>
      ...
      {isQuestionBankOpen && (
        <QuestionBankModal
          ...
          existingQuestions={testData.codingChallenges}
        />
      )}
    </div>
  );
