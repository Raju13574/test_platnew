...
const MCQSection = ({ testData, setTestData, renderOptionText }) => {
  ...
  const handleSelectFromQuestionBank = (selectedQuestions) => {
    setTestData(prev => ({
      ...prev,
      mcqs: [...prev.mcqs, ...selectedQuestions]
    }));
    toast.success(`Added ${selectedQuestions.length} questions from Question Bank`);
  };
    ...
    <div className="space-y-6">
                    ...
                    <div className="space-y-1">
                      <div className="options-list">
                        {mcq.options.map((option, optionIndex) => (
                          <div key={optionIndex}>
                            {renderOptionText ? renderOptionText(option) : option}
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {isQuestionBankOpen && (
        <QuestionBankModal
          ...
          existingQuestions={testData.mcqs}
        />
      )}
    </div>
  );
