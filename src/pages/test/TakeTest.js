...
export default function TakeTest() {
  ...
  // Move handleSubmit definition before handleWarning
  ...
  // Now define handleWarning after handleSubmit
  const handleWarning = useCallback((message) => {
    const now = Date.now();
    const lastWarning = lastWarningRef.current;
    const WARNING_COOLDOWN = 5000;
    const maxWarnings = test?.warningSystem?.maxWarnings || 3; // Get from test config
    ...
    if (now - lastWarning >= WARNING_COOLDOWN) {
      ...
      updateAnalytics(prev => {
        ...
        // Check against configured max warnings
        if (test?.warningSystem?.enabled && newWarnings >= maxWarnings) {
          // Check if autoSubmit is enabled in warning system
          if (test?.warningSystem?.actions?.autoSubmit) {
            // Update test status first
            apiService.post('submissions/update-status', {
              testId: testId,
              status: 'completed'
            }).then(() => {
              toast.error(`Maximum warnings (${maxWarnings}) exceeded. Test will be submitted.`);
              // Force navigate to completion page
              navigate('/test/completed', { 
                state: { 
                  testId: uuid,
                  submission: {
            ...
            // If autoSubmit is disabled, just show a warning
            toast.error(`Warning ${newWarnings}/${maxWarnings}: Maximum warnings reached!`);
          }
        }
  ...
  }, [updateAnalytics, testId, uuid, answers, test, navigate]);
  ...
  // Update the useEffect for requesting permissions to check proctoring flag
  useEffect(() => {
    const requestPermissions = async () => {
      // Only request camera if proctoring is enabled
      if (test?.proctoring?.enabled === true) {
        try {
  ...
  // Update handleConfirmedSubmit to clear ALL test-related items
  const handleConfirmedSubmit = async () => {
    ...
    try {
      ...
      const itemsToClear = [
        ...
        'testStarted',
        'testStartTime',
        'codingSubmissionStatus', // Added new items
        'currentVendorId',
        'testStatus',
      ...
      // Additionally clear any items that start with specific prefixes
  ...
  const SetupStepContent = ({ step, onNext }) => {
    ...
    switch (step) {
      ...
      case 1:
        // Skip camera setup if proctoring is disabled
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    toast.error('Camera access required to continue');
                  }
                }}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Enhanced Header with integrated proctoring */}
      <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-lg">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-start py-4">
            {/* Test Info with enhanced typography */}
            <div>
              ...
              {test && (
                <div className="flex items-center gap-4 mt-2">
                  <div className="flex items-center text-gray-700 font-medium">
                    ...
                    <span className="font-mono text-lg bg-blue-50 px-3 py-1 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-200
                      ${isFullScreen ? 'bg-green-100 text-green-700 ring-1 ring-green-400' : 'bg-red-100 text-red-700 ring-1 ring-red-400'}`}>
                      {isFullScreen ? '✓ Fullscreen' : '⚠ Not Fullscreen'}
                    </span>
                    <span className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-200
                      ${isTabVisible ? 'bg-green-100 text-green-700 ring-1 ring-green-400' : 'bg-red-100 text-red-700 ring-1 ring-red-400'}`}>
                      {isTabVisible ? '✓ Tab Active' : '⚠ Tab Inactive'}
                    </span>
                    <span className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors duration-200
                      ${isWindowFocused ? 'bg-green-100 text-green-700 ring-1 ring-green-400' : 'bg-red-100 text-red-700 ring-1 ring-red-400'}`}>
            ...
            {/* Enhanced action buttons and camera */}
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="px-6 py-2.5 flex items-center gap-2 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 
              ...
              {/* Only show proctoring if enabled */}
              {test?.proctoring?.enabled && (
                <div className="w-[180px] h-[135px] bg-black rounded-lg overflow-hidden shadow-xl ring-1 ring-gray-200">
                  <Proctoring
                    testId={testId}
                    onWarning={(message) => {
                      setWarningMessage(message);
                      ...
                      // Update analytics for the warning
                      updateAnalytics(prev => {
                        const newWarnings = prev.warnings + 1;
                        
                        // If warning threshold exceeded, force complete the test
                        if (newWarnings >= 3) {
                          // First update test status
                          apiService.post('submissions/update-status', {
                            testId: testId,
                            status: 'completed'
                          }).then(() => {
                            toast.error('Maximum warnings exceeded. Test will be submitted.');
                            // Force navigate to completion page
                            navigate('/test/completed', { 
                              state: { 
                                testId: uuid,
          ...
          <div className="flex space-x-1 mt-2">
            <button
              ...
              className={`px-6 py-2.5 text-sm rounded-t-lg font-medium transition-all relative
                ${currentSection === 'mcq' 
            ...
            >
              MCQ Section
              <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-blue-50 text-blue-700 font-semibold">
                {test.mcqs?.length || 0}
              </span>
            </button>
            <button
      ...
      <div className="flex-grow flex bg-white">
        <div className="w-full px-0 shadow-inner">
          {currentSection === 'mcq' ? (
            <MCQSection
              mcqs={test?.mcqs || []}
              answers={answers.mcq}
      ...
      {showWarningModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl transform transition-all duration-300">
            <WarningModal
              message={warningMessage}
              warningCount={analytics.warnings}
              maxWarnings={test?.warningSystem?.maxWarnings || 3}
              onClose={() => {
          ...
          <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all duration-300">
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-4">
                <li className="flex items-center p-3 rounded-lg bg-gray-50">
                  <CheckCircle 
                    className={`w-6 h-6 mr-3 ${test?.mcqSubmission ? 'text-green-500' : 'text-gray-300'}`}
                  />
                  <span className="text-gray-700 font-medium">MCQ Section</span>
                </li>
                <li className="flex items-center p-3 rounded-lg bg-gray-50">
                  <CheckCircle 
                    className={`w-6 h-6 mr-3 ${test?.codingSubmission ? 'text-green-500' : 'text-gray-300'}`}
                  />
                  <span className="text-gray-700 font-medium">Coding Section</span>
                </li>
              </ul>
              <p className="text-sm text-gray-500 mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-100">
            ...
            <div className="flex justify-end gap-4 mt-8">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-lg font-medium
                  hover:from-green-700 hover:to-green-600 transition-all duration-300 transform hover:scale-105 hover:shadow-lg"
              >
                Confirm Submit
              </button>
            </div>
